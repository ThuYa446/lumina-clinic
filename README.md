...
2. Enter fictional contact details, client ID number and date of birth. Leave membership blank to reserve a 15-minute payment hold.
...
5. Open **Clinic team** (`/staff`). Local credentials: `receptionist` / `local-reception-only`. Select the date, expand **Consent details** on a booking to view its client ID number and date of birth, and record a simulated refund reference when applicable. Sign out to clear in-memory credentials.
...
New bookings also save the client ID number and date of birth for the clinic's consent records. Migration V7 adds these per-booking fields without inventing values for existing bookings; legacy records can have neither value. Authenticated clinic staff can view them in the diary. Booking creation and management responses do not return these fields, and collecting them does not record signed treatment consent.
...
| R7 | Recheck and allocate under branch lock; DB rejects overlap independently | [BookingService:175](src/main/java/com/lumina/clinic/booking/BookingService.java#L175); [BranchRepository:9](src/main/java/com/lumina/clinic/catalog/BranchRepository.java#L9); [V1 schema:73](src/main/resources/db/migration/V1__create_booking_schema.sql#L73) |
| R8 | Nonmember 300 deposit; 15-minute hold; expires exactly at deadline | [Booking:51](src/main/java/com/lumina/clinic/booking/Booking.java#L51); [Booking:61](src/main/java/com/lumina/clinic/booking/Booking.java#L61); [BookingService:185](src/main/java/com/lumina/clinic/booking/BookingService.java#L185) |
...
| R10 | Creation UUID retry key; identical payload replays, changes conflict | [BookingController:27](src/main/java/com/lumina/clinic/booking/BookingController.java#L27); [BookingService:81](src/main/java/com/lumina/clinic/booking/BookingService.java#L81); [BookingService:216](src/main/java/com/lumina/clinic/booking/BookingService.java#L216) |
| R11 | Private active hold payment; one receipt per booking under retries | [BookingService:113](src/main/java/com/lumina/clinic/booking/BookingService.java#L113); [PaymentTransaction:13](src/main/java/com/lumina/clinic/payment/PaymentTransaction.java#L13); [V1 schema:85](src/main/resources/db/migration/V1__create_booking_schema.sql#L85) |
...
| R15 | Validated contact fields plus required client ID number (up to 64 characters) and nonfuture DOB in clinic time; reject unknown fields | [CreateBookingRequest:23](src/main/java/com/lumina/clinic/booking/CreateBookingRequest.java#L23); [V7 schema:3](src/main/resources/db/migration/V7__store_client_identity.sql#L3); [ApplicationConfiguration:17](src/main/java/com/lumina/clinic/config/ApplicationConfiguration.java#L17); [application.yml:24](src/main/resources/application.yml#L24) |
<!-- RULE_INDEX_END -->
...
| Clinical consent workflow | Client ID number and DOB are stored and available to authenticated staff. Consent-form completion/signing, consent ownership and retention/deletion automation remain unfinished. |
